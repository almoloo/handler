...
Plain `AggregatorV3` price reads (§4.4) don't qualify for any open ETHOnline 2026 Chainlink prize — the only non-Continuity track is "Best Confidential Workflow" ($2,000, up to 2 teams), which requires a CRE workflow that registers a confidential TEE handler (`handlerInTee` / `cre.HandlerInTee`) processing at least one genuinely sensitive input inside the enclave, demonstrated via `cre workflow simulate` or a live deployment. Trust tiers and policy thresholds are public on-chain data, so wrapping those would read as a placeholder. The one genuinely sensitive thing in Riley's loop is the **1inch API key**: the candidate stretch is moving Riley's quote step into a CRE workflow whose TEE handler calls the 1inch Swap API through the SDK's `ConfidentialHTTPClient` with the key as a vault DON secret, returning the quote to the api, which then submits `tryExecute()` as today. Caveats, both real: `ConfidentialHTTPClient` and `cre workflow simulate` are documented in `@chainlink/cre-sdk`, but `handlerInTee` is new enough that it isn't in the docs yet, and Chainlink's own concepts page says Confidential Workflows are in **private beta, enrolled through a Chainlink account team**. The prize accepts CLI-simulation evidence, so the day-8 spike is strictly: (a) can we get access or does simulation work without enrollment, (b) does `handlerInTee` exist in the installed SDK version. Either answer "no" ⇒ drop it the same day, no sunk cost. Attempt only after day 7's feature freeze on the locked scope, and drop it first if it threatens anything in §7's "never cut" list.
