...
Full product spec: `context/project-overview.md`. The product is built out end-to-end per the three roadmaps, not scaffolding: `packages/contracts` has the full contract set (`HandlerWallet`, `TrustReader`, `PriceConverter`, `HandlerWalletFactory`) with real ERC-8004 and Chainlink integrations and no stubs left in deployable code; `apps/api` has its module set built (see Architecture below), missing only a `prices/` module and the full `demo/` director (currently just the villain actor — no `POST /demo/beat/:n` or `/demo/reset` yet); `apps/web` has its full design-system component library plus the Payroll, hire, activity, and approval-sheet (Ledger co-sign) screens, missing the agent-file screen (`/app/agent/[id]`). See `context/current-feature.md` for what's actively in flight.
...
- **`apps/api`** (NestJS + Prisma/Postgres + viem): reads chain state via a cron-based indexer (no queue infra) and serves it over REST/SSE. Module split (per `context/coding-standards.md`), now mostly built: `chain/`, `indexer/`, `agents/`, `trust/`, `policies/`, `activity/`, `approvals/`, `demo/` (villain actor only so far), `health/` — `prices/` doesn't exist yet. Dependency rule: `agents`/`indexer`/`demo` may depend on `chain`; `activity`/`policies`/`trust`/`prices` are read-only serve modules. Prisma is the only DB access layer. Chain-derived rows must be idempotent by a natural key (`txHash`, deterministic `PendingApproval.id`) so the indexer can safely re-run.
...
- **Product-real, not demo-only**: Handler must work for any wallet that signs in, not just a showcase wallet. No mock stores, fixtures, seeded trust overrides, hardcoded prices, or "demo mode" branches in runtime code (test doubles live in test files only). Every "blocked"/"approved" moment shown in the app or demo video is a real on-chain tx through the real checks. `/demo` and `POST /demo/reset` are env-gated operator tooling that only trigger real agent actions against the showcase wallet, kept separate from user-facing code paths. See `context/project-overview.md` "Product integrity".
- **Auth on reads too**: every `/app`-serving API route (reads and writes) requires a SIWE session and is scoped to the session's wallet; only `/health`, `/prices`, `/auth/*` are public.
